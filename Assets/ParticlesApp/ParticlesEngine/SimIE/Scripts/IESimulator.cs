...
public class IESimulator : MonoBehaviour {
    ...
    private void FixedUpdate() {
        var didDisplayMove = _prevDisplayPosition != _manager.displayAnchor.position ||
                             _prevDisplayRotation != _manager.displayAnchor.rotation ||
                             _prevDisplayScale != _manager.displayAnchor.localScale;
        if (didDisplayMove) {
            var originalTransform = Matrix4x4.TRS(_prevDisplayPosition,
                _prevDisplayRotation,
                _prevDisplayScale);
      
            var newTransform = Matrix4x4.TRS(_manager.displayAnchor.position,
                _manager.displayAnchor.rotation,
                _manager.displayAnchor.localScale);
      
            var delta = newTransform * originalTransform.inverse;
            TransformBy(delta);
        ...
        var didParticleSizeChange = _prevParticleSize != _manager.particleRadius;
        ...
        var scale = _manager.displayAnchor.localScale.x;

        foreach (var particle in _particles) {
            var collisionForce = Vector3.zero;
            var socialForce = Vector3.zero;
            var socialInteractions = 0;

            foreach (var other in _particles) {
                if (other == particle) continue;

                var toOther = other.rigidbody.position - particle.rigidbody.position;
                var distance = toOther.magnitude;
                toOther = distance < 0.0001 ? Vector3.zero : toOther / distance;

                if (distance < PARTICLE_DIAMETER * scale) {
                    var penetration = 1 - distance / (PARTICLE_DIAMETER * scale);
                    var collisionScalar = (currentDescription.speciesData[particle.species].collisionForce + currentDescription.speciesData[other.species].collisionForce) * 0.5f;
            ...
            var toCenter = _manager.fieldCenter - _manager.displayAnchor.InverseTransformPoint(particle.rigidbody.position);
            var distToCenter = toCenter.magnitude;
            ...
            var simVelocity = _manager.displayAnchor.InverseTransformVector(particle.rigidbody.velocity) * Time.fixedDeltaTime;
